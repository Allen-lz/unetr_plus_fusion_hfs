...
class UNETR_PP(SegmentationNetwork):
    ...
    def extract_cross_hfs(self, enc):
        ...
        space_hfs_map = self.hfs.run(space_hfs_enc).unsqueeze(dim=1)
        channel_hfs_enc = torch.mean(torch.mean(enc, dim=[-2, -1], keepdim=False), dim=2, keepdim=False)
    ...
    def forward(self, x_in, pharse="train"):

        # ================================================================================
        # x_in.shape = torch.Size([4, 1, 16, 160, 160])
        b, c, f, h, w = x_in.shape
        d_x_in = x_in.permute(0, 2, 1, 3, 4).reshape(b * f, c, h, w)

        # 将d_x_in输入到Decouple中进行解耦之后得到重建出来的结果和高低频的信息
        ...
        # Four decoders
        """
        dec4 shape torch.Size([4, 256, 2, 5, 5])
        dec3 shape torch.Size([4, 128, 4, 10, 10])
        dec2 shape torch.Size([4, 64, 8, 20, 20])
        dec1 shape torch.Size([4, 32, 16, 40, 40])
        """
